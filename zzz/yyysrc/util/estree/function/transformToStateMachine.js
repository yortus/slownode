...
function transformToStateMachine(func) {
    // Validate arguments.
    assert(func.params.every(function (p) { return p.type === 'Identifier'; }));
    assert(func.body.type === 'BlockStatement');
    // Construct a Rewriter instance to handle the rewrite operation.
    var rewriter = new Rewriter(func.body, func.params);
    // Extract and return the rewritten AST.
    var newFunc = rewriter.generateAST();
    return newFunc;
}
...
var Rewriter = (function () {
    ...
    function Rewriter(body, params) {
        ...
        traverseTree(body, function (node) {
            matchNode(node, {
    ...
    matchNode(stmt, {
        ...
        ForInStatement: function (stmt) {
            ...
            if (loopVar.type !== 'Identifier')
                throw new Error("Steppable: unsupported for..in loop variable type: '" + loopVar.type + "'");
            if (stmt.left.type === 'VariableDeclaration') {
                if (stmt.left['declarations'][0].init)
                    throw new Error("Steppable: for..in loop variable initialiser is not supported.");
            }
        ...
        TryStatement: function (stmt) {
            ...
            if (stmt.handler) {
                if (stmt.handler.param.type !== 'Identifier')
                    throw new Error("Steppable: catch parameter must be an identifier");
        ...
        VariableDeclaration: function (stmt) {
            stmt.declarations.forEach(function (decl) {
                if (decl.id.type !== 'Identifier')
                    throw new Error("Steppable: unsupported declarator ID type: '" + decl.id.type + "'");
...
module.exports = transformToStateMachine;
//# sourceMappingURL=transformToStateMachine.js.map