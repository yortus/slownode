...
import flushLogs = require('../flushLogs');


// TODO: doc...
export function enqueue(entry: EventLoopEntry) {
    entries.push(entry);
}


// TODO: doc...
export function remove(entry: EventLoopEntry) {
...
function runLoopForever() {
    var timer = setInterval(() => {
        ...
        setImmediate(() => {
            if (entries.length === 0) clearInterval(timer);
        });
